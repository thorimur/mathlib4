...
definitions, but excludes decls without "bodies" (such as `alias`es, `structure`s, declarations
generated by atttributes like `@[ext]`, and so on). Assumes that every body elaboration proceeds
through `Lean.Elab.Term.BodyInfo`.
-/
